...
export const listWardrobeItems = query({
  ...
  handler: async (ctx) => {
    ...
    return projectWardrobeItems(ctx, userId, items);
  },
});
...
export const getUploadUrl = mutation({
  ...
  handler: async (ctx) => {
    ...
    return Effect.runPromise(issueUploadTicket(ctx, userId));
  },
});
...
export const createWardrobeItem = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await requireOwnedStorage(ctx, userId, args.storageId);
...
export async function projectWardrobeItems(ctx: QueryCtx, userId: string, items: Doc<"wardrobeItems">[]) {
    ...
    return withUrls.flatMap(item => item.imageUrl ? [{ ...item, imageUrl: item.imageUrl }] : []);
}

export const pageWardrobeItems = query({
  args: { paginationOpts: paginationOptsValidator },
  handler: async (ctx, { paginationOpts }) => {
    const userId = await getAuthenticatedUserId(ctx);
    if (!userId) return { page: [], isDone: true, continueCursor: "" };
    const result = await ctx.db.query("wardrobeItems").withIndex("by_user_createdAt", q => q.eq("userId", userId)).order("desc").paginate({ ...paginationOpts, numItems: Math.max(1, Math.min(100, paginationOpts.numItems)), maximumRowsRead: 100 });
    return { ...result, page: await projectWardrobeItems(ctx, userId, result.page) };
  },
});
