...
import type { StyleFitResponse } from "@wardrobe/shared";
...
export default function HomeScreen() {
  ...
  const checkFit = async () => {
    ...
    try {
      ...
      setResult(await client.checkStyleFit({ imageUrl, title }));
      setStatus(null);
    } catch (error) {
    ...
    <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={{ padding: 20, gap: 16 }}>
      <View style={{ gap: 8 }}>
        ...
        <Text selectable style={{ color: "#475569", lineHeight: 22 }}>
          Paste a product image URL to check a piece against the Wardrobe context layer.
        </Text>
      </View>
