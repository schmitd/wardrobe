...
export function ItemDetailScreen() {
  ...
  const previewMutation = useMutation({
    mutationFn: async () => { const result = await changePreview(getToken, itemId, "request_preview"); if (!result.success) throw new Error("Preview is unavailable. Please try again later."); },
  ...
  return (
    <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={{ padding: 20, paddingBottom: 70, gap: 18 }}>
      ...
      {preview.data?.enabled && preview.data.status !== "ready" ? <View style={{ gap: 8 }}>
        {["queued", "processing"].includes(preview.data.status) ? <Text accessibilityLiveRegion="polite">Preparing photo…</Text> : <>
          {preview.data.status === "skipped" ? <Text>Try a clearer photo of this piece.</Text> : preview.data.status === "error" ? <Text>Couldn’t prepare this photo.</Text> : null}
          {preview.data.status !== "skipped" ? <Pressable accessibilityRole="button" disabled={previewMutation.isPending || item.analysisStatus !== "ready"} onPress={() => previewMutation.mutate()} style={{ minHeight: 48, justifyContent: "center" }}><Text style={{ color: colors.plum, fontWeight: "800" }}>{preview.data.status === "error" ? "Try again" : "Prepare photo"}</Text></Pressable> : null}
        </>}
