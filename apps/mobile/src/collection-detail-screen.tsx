...
import { loadCollection, addCollectionItem, removeCollectionItem, saveInspiration } from "@/api";
...
export function CollectionDetailScreen() {
  ...
  const detail = useInfiniteQuery({ queryKey: ["mobile-collection", userId, collectionId], queryFn: ({ pageParam }) => loadCollection(getToken, collectionId, pageParam), initialPageParam: null as string | null, getNextPageParam: last => last?.cursor ?? undefined, enabled: Boolean(isSignedIn && collectionId) });
  const first = detail.data?.pages[0];
  const collection = first ? { ...first, items: detail.data!.pages.flatMap(page => page?.items ?? []), inspirations: detail.data!.pages.flatMap(page => page?.inspirations ?? []) } : null;
  ...
  const changeMembership = useMutation({
    ...
    onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ["mobile-collection", userId, collectionId] }); await queryClient.invalidateQueries({ queryKey: ["mobile-bootstrap"] }); },
  });
  ...
  if (detail.error) return <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={{ padding: 20 }}><ErrorPanel message={detail.error.message} retry={() => void detail.refetch()} /></ScrollView>;
  if (!collection) return <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={{ padding: 20 }}><ErrorPanel message="This plan could not be found." /></ScrollView>;
  ...
  return (
    <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={{ padding: 20, paddingBottom: 80, gap: 24 }}>
      ...
      <View style={{ gap: 12 }}><Text selectable style={{ color: colors.ink, fontSize: 21, fontWeight: "900" }}>In this plan</Text>{collection.items.length ? <View style={{ gap: 10 }}>{collection.items.map((membership) => <View key={membership._id} style={{ flexDirection: "row", alignItems: "center", gap: 12, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.surface, padding: 9, borderRadius: 12, borderCurve: "continuous" }}><Image source={membership.item.imageUrl} style={{ width: 64, height: 64, backgroundColor: colors.wash, borderRadius: 8 }} contentFit="cover" /><View style={{ flex: 1, gap: 3 }}><Text selectable numberOfLines={1} style={{ color: colors.ink, fontWeight: "900" }}>{membership.item.category ?? "Wardrobe piece"}</Text><Text selectable numberOfLines={1} style={{ color: colors.muted, fontSize: 12 }}>{membership.item.description ?? "Owned"}</Text></View><Pressable accessibilityLabel={`Remove ${membership.item.category ?? "piece"} from plan`} disabled={changeMembership.isPending} onPress={() => changeMembership.mutate({ itemId: membership.item.id, remove: true })} style={{ width: 40, height: 40, alignItems: "center", justifyContent: "center" }}><MaterialCommunityIcons name="close" size={22} color={colors.danger} /></Pressable></View>)}</View> : <Text selectable style={{ color: colors.muted }}>No owned pieces here yet.</Text>}</View>

      {available.length ? <View style={{ gap: 12 }}><View><Text selectable style={{ color: colors.ink, fontSize: 21, fontWeight: "900" }}>Add from your wardrobe</Text><Text selectable style={{ color: colors.muted, fontSize: 13 }}>Tap a piece to connect it to this plan.</Text></View><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 10 }}>{available.map((item) => <Pressable key={item.id} disabled={changeMembership.isPending} onPress={() => changeMembership.mutate({ itemId: item.id })} style={{ width: 112, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.surface, overflow: "hidden", borderRadius: 12, borderCurve: "continuous" }}><Image source={item.imageUrl} style={{ width: "100%", aspectRatio: 1 }} contentFit="cover" /><View style={{ padding: 8, gap: 3 }}><Text numberOfLines={1} style={{ color: colors.ink, fontSize: 12, fontWeight: "900" }}>{item.category ?? "Piece"}</Text><Text style={{ color: colors.plum, fontSize: 11, fontWeight: "800" }}>+ Add</Text></View></Pressable>)}</ScrollView></View> : null}
      {detail.hasNextPage ? <Pressable disabled={detail.isFetchingNextPage} onPress={() => void detail.fetchNextPage()} style={{ padding: 16 }}><Text>Load more plan pieces and inspiration</Text></Pressable> : null}
      {query.data?.closetCursor ? <Pressable disabled={query.loadingMore} onPress={() => query.loadMore("closet")} style={{ padding: 16 }}><Text>Load more closet pieces</Text></Pressable> : null}
