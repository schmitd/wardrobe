...
export function NewCollectionScreen() {
  ...
  return (
      <ScrollView style={{ flex: 1, backgroundColor: colors.paper }} contentInsetAdjustmentBehavior="automatic" automaticallyAdjustKeyboardInsets keyboardDismissMode="on-drag" keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: 20, gap: 20 }}>
        ...
        <Pressable disabled={!name.trim() || mutation.isPending} onPress={() => { Keyboard.dismiss(); mutation.mutate(); }} style={{ minHeight: 52, backgroundColor: colors.lime, borderColor: colors.line, borderWidth: 1, alignItems: "center", justifyContent: "center", opacity: !name.trim() || mutation.isPending ? 0.55 : 1, borderRadius: 10, borderCurve: "continuous" }}>{mutation.isPending ? <ActivityIndicator color={colors.ink} /> : <Text style={{ color: colors.ink, fontWeight: "900", fontSize: 16 }}>Create plan</Text>}</Pressable>
      </ScrollView>
  );
}
