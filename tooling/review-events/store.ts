...
export class Store {
  ...
  receive(delivery: string, digest: string, event: Event): boolean {
    return this.db.query("INSERT OR IGNORE INTO events (delivery,digest,event) VALUES (?,?,?)").run(delivery, digest, JSON.stringify(event)).changes === 1;
  }
  trust(pr: number, head: string) { this.db.query("INSERT OR IGNORE INTO trusted_heads VALUES (?,?)").run(pr, head); }
  ...
  finish(id: number, state: "done" | "failed", detail: string) { this.db.query("UPDATE events SET state=?,finished=CURRENT_TIMESTAMP,detail=? WHERE id=?").run(state, detail.slice(0, 4000), id); }
  close() { this.db.close(); }
}
