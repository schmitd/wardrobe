...
The controller uses GitHub's current API state, not a webhook conclusion, to decide readiness. GitHub Codex review activity must be complete, but its opening review need not rerun on every push: the fresh independent review checks the exact new revision and every current feedback thread. It requires `Merge checks`, `Wardrobe adversarial`, strict up-to-date checks, and admin enforcement. It calls only the installed trusted merge publisher. Missing checks remain pending. No protection bypass is provided.
