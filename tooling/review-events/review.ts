...
export type Report = { base: string; head: string; verdict: string; summary: string; findings: string[]; decisions: string[]; coverageGaps: string[]; commands: { command: string; exitCode: number; artifact: string }[]; threads: { id: string; fingerprint?: string; disposition: "fixed" | "not_applicable" | "unresolved"; reason: string; evidence: string }[] };

export function threadFingerprint(thread: Thread): string {
  return createHash("sha256").update(JSON.stringify({ path: thread.path, comments: thread.comments })).digest("hex");
}

export function coversThreads(report: Report, threads: Thread[]): boolean {
  return Array.isArray(report.threads) && threads.every(thread => report.threads.filter(t => t.id === thread.id).length === 1 && report.threads.some(t => t.id === thread.id && t.fingerprint === threadFingerprint(thread) && ["fixed", "not_applicable"].includes(t.disposition) && t.reason.trim() && t.evidence.trim()));
}
