...
export interface GeminiService {
    ...
    readonly embedContent: (
        text: string
    ) => Effect.Effect<EmbedContentResponse, GeminiError>
...
const make = Effect.gen(function* () {
    ...
    // Helper to choose model based on name (though we only have one main multimodality model now effectively)
    const getModel = (name: string) => {
        // In strict sense we can just use one model instance for both text/vision if it's the same model
        return visionModel
    }

    return {
        generateContent: (modelName: string, request: GenerateContentRequest | string | Array<string | any>) =>
            Effect.tryPromise({
                try: () => getModel(modelName).generateContent(request),
                catch: (e) => new GeminiError(e)
            }).pipe(
                Effect.withSpan("gemini.generateContent", { attributes: { model: modelName } })
            ),
        ...
        embedContent: (text: string) =>
            Effect.tryPromise({
                ...
                Effect.withSpan("gemini.embedContent")
            ),
        ...
        batchEmbedContents: (request: BatchEmbedContentsRequest) =>
            Effect.tryPromise({
                ...
                Effect.withSpan("gemini.batchEmbedContents")
            )
    }
})
