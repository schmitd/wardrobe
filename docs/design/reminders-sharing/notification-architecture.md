...
Proposed account-level controls: cap 2 reminders per local day, minimum 3 hours apart, quiet hours 21:00–09:00. Event-start reminders that collide with quiet hours/cooldown are skipped, not delivered later out of context. Earliest due event wins a tie, then stable occurrence ID; others record a suppression reason. These numbers are proposals awaiting David's feedback. The once-per-day midday requirement is fixed; caps and timing are not hard-coded into screens.
