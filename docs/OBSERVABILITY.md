...
Capture localization emits Axiom-only `fit_check.localization.complete` with trace context, detector version/model, automatic-zoom boolean, detected/initially-rejected/accepted counts, and duration. Use these to compare first-pass crop acceptance, automatic repair rate, and latency. No source photos, boxes, descriptions, prompts or verifier responses go into telemetry. Local private regression artifacts are ignored by Git and never uploaded as issue attachments. Model acceptance is a proxy, not human-verified crop accuracy.

Luna generation adds `model`, `inputTokens`, `outputTokens`, `cachedTokens`, `cacheWriteTokens`, and `estimatedCostUsd` to the Axiom `openai.generateContent` span. Transcription uses `openai.transcribe` with its model name. No prompt, output text, photo, audio, cache key, or provider error body is attached. Cost estimates and the explicit cache policy are documented in [Luna inference](LUNA_INFERENCE.md); PostHog receives none of these operational payloads.
